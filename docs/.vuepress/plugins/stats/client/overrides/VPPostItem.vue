...
import { isMobile as detectMobile } from '@vuepress/helper/client'
...
const readingTime = computed(() => {
  ...
  const result = readingTimeLocale.value ? getReadingTimeLocale(post.readingTime, readingTimeLocale.value) : fallback
  result.time = result.time.replace(/^\D+/, '')
  return result
})

const sticky = computed(() => {
  if (typeof post.sticky === 'boolean') return post.sticky
  if (typeof post.sticky === 'number') return post.sticky >= 0
  return false
})
...
const tags = computed(() => {
  ...
  return (post.tags ?? []).slice(0, 4).map(tag => ({
    name: tag,
...
const cover = computed<PostsCoverStyle | null>(() => {
  ...
  const option = collection.value?.postCover ?? 'right'
  const options = typeof option === 'string' ? { layout: option } : option
...
const coverLayout = computed(() => {
  ...
  if (layout === 'odd-left') return odd ? 'left' : 'right'
  if (layout === 'odd-right') return odd ? 'right' : 'left'
  return layout
})
...
const coverStyles = computed(() => {
  ...
  if (typeof cover.value.ratio === 'number') {
    ...
    const [width, height] = cover.value.ratio!.split(/[:/]/).map(Number)
    ratio = height / width
  }

  if (coverLayout.value === 'left' || coverLayout.value === 'right') {
    const width = cover.value.width ?? 240
    ...
    <div class="post-item-content">
      ...
      <div class="post-meta">
        <div v-if="categoryList.length" class="category-list">
          ...
          <template v-for="(category, itemIndex) in categoryList" :key="itemIndex">
            <VPLink :href="categoriesLink ? `${categoriesLink.link}?id=${category.id}` : undefined" no-icon>
              {{ category.name }}
            </VPLink>
            <span v-if="itemIndex !== categoryList.length - 1">/</span>
          </template>
        </div>
